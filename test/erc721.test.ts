...
import { TokenConfig, HypERC721Config, HypERC721CollateralConfig } from '../src/config';
import { HypERC721Contracts } from '../src/contracts';
import { HypERC721Deployer } from '../src/deploy';
import { ERC721, ERC721Test__factory, ERC721__factory, HypERC721, HypERC721Collateral } from '../src/types';
...
describe(`HypERC721${withCollateral ? 'Collateral' : ''}`, async () => {
  ...
  beforeEach(async () => {
    ...
    const configWithTokenInfo: ChainMap<TestChainNames, HypERC721Config | HypERC721CollateralConfig> =
    ...
    if (withCollateral) {
      erc721 = await new ERC721Test__factory(owner).deploy(tokenConfig.name, tokenConfig.symbol, tokenConfig.totalSupply);
  ...
  it('should not be initializable again', async () => {
    const initializeTx = withCollateral ? (local as HypERC721Collateral).initialize(
    ...
    await expect(initializeTx).to.be.revertedWith('Initializable: contract is already initialized');
  });
  ...
  it('should allow for local transfers', async () => {
    ...
    await (local as HypERC721).transferFrom(owner.address, recipient.address, tokenId);
  ...
  it('should not allow transfers of nonexistent identifiers', async () => {
    ...
    if (!withCollateral) {
      await expect(
        (local as HypERC721).transferFrom(owner.address, recipient.address, invalidTokenId),
  ...
  it('should prevent remote transfer of unowned id', async () => {
    const revertReason = withCollateral ? 'ERC721: transfer from incorrect owner' : '!owner';
    await expect(
      local.connect(recipient).transferRemote(remoteDomain, utils.addressToBytes32(recipient.address), tokenId2)
