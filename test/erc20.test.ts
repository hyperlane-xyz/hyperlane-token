...
import { HypERC20CollateralConfig, HypERC20Config, TokenConfig } from '../src/config';
import { HypERC20Contracts } from '../src/contracts';
import { HypERC20Deployer } from '../src/deploy';
import { ERC20, ERC20Test__factory, ERC20__factory, HypERC20, HypERC20Collateral } from '../src/types';
...
describe(`HypERC20${withCollateral ? 'Collateral' : ''}`, async () => {
  ...
  beforeEach(async () => {
    ...
    const configWithTokenInfo: ChainMap<TestChainNames, HypERC20Config | HypERC20CollateralConfig> =
      ...
      if (withCollateral) {
        erc20 = await new ERC20Test__factory(owner).deploy(tokenConfig.name, tokenConfig.symbol, tokenConfig.totalSupply);
  ...
  it('should not be initializable again', async () => {
    const initializeTx = withCollateral ? (local as HypERC20Collateral).initialize(
  ...
  it('should prevent remote transfer of unowned balance', async () => {
    const revertReason = withCollateral ? 'ERC20: insufficient allowance' : 'ERC20: burn amount exceeds balance'; 
    await expect(
      local.connect(recipient).transferRemote(remoteDomain, utils.addressToBytes32(recipient.address), amount)
